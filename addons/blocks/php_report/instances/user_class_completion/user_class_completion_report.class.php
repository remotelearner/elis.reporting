...
class user_class_completion_report extends table_report {
    ...
    function require_dependencies() {
        ...
        require_once($CFG->dirroot . '/curriculum/lib/filtering/elisuserprofile.php');
        require_once($CFG->dirroot . '/curriculum/lib/filtering/curriculumclass.php');

        // Reference the "child" report
        require_once($CFG->dirroot.'/blocks/php_report/instances/user_class_completion_details'.
                                   '/user_class_completion_details_report.class.php');
    ...
    function get_filters() {
        ...
        $clustertree_help = array(
                'user_class_completion_cluster_help',
                $help_label,
                'block_php_report');
        ...
        $completionstatus_options = array(
                ...
                'numeric' => true,
                'help' => array('user_class_completion_completionstatus_help',
                                get_string('completion_status', 'block_curr_admin'),
                                'block_php_report'));
        ...
        $userfilter =
            new generalized_filter_elisuserprofile(
                ...
                array(
                    ...
                    'help'        => array(
                        'up' => array(
                            'fullname'  => array('user_class_completion_fullname_help', get_string('fullname'), 'block_php_report'),
                            'lastname'  => array('user_class_completion_lastname_help', get_string('lastname'), 'block_php_report'),
                            'firstname' => array('user_class_completion_firstname_help', get_string('firstname'), 'block_php_report'),
                            'idnumber'  => array('user_class_completion_idnumber_help', get_string('idnumber'), 'block_php_report'),
                            'email'     => array('user_class_completion_email_help', get_string('email'), 'block_php_report'),
                            'city'      => array('user_class_completion_city_help', get_string('city'), 'block_php_report'),
                            'country'   => array('user_class_completion_country_help', get_string('country'), 'block_php_report'),
                            'username'  => array('user_class_completion_username_help', get_string('username'), 'block_php_report'),
                            'language'  => array('user_class_completion_language_help', get_string('language'), 'block_php_report'),
                            'inactive'  => array('user_class_completion_inactive_help', get_string('inactive'), 'block_php_report'),
                        ),
                    ),
                    'outerfield'  => array(
                        'up' => 'u.id',
        ...
        $classfilter =
            new generalized_filter_curriculumclass(
                ...
                array(
                    ...
                    'help'        => array(
                        'curriculum' => array(
                            'name' => array('user_class_completion_curriculum_help',
                                            get_string('curriculum', 'block_curr_admin'),
                                            'block_php_report'),
                        ),
                        'course' => array(
                            'name' => array('user_class_completion_course_help',
                                            get_string('course', 'block_curr_admin'),
                                            'block_php_report')
                        ),
                        'class' => array(
                            'idnumber'          => array('user_class_completion_class_help',
                                                         get_string('c', 'block_curr_admin'),
                                                         'block_php_report'),
                            'environmentid'     => array('user_class_completion_environment_help',
                                                         get_string('environment', 'block_curr_admin'),
                                                         'block_php_report'),
                            'startdate'         => array('user_class_completion_startdate_help',
                                                         get_string('startdate', 'block_curr_admin'),
                                                         'block_php_report'),
                        ),
                    ),
                    'wrapper'     => array(
                        'curriculum' => '',
                        'course'     => " INNER JOIN {$CFG->prefix}crlm_class_enrolment clse"
                                      . " ON clse.classid = cls.id",
                        'class'      => " INNER JOIN {$CFG->prefix}crlm_class_enrolment clse"
                                      . " ON clse.classid = cls.id",
                     ),
                    'innerfield'  => array(
        ...
        $titleoptions = array(
            'help' => array('report_title', get_string('report_title', $this->languagefile), 'block_php_report'),
        );
        ...
        $summarycoloptions = array(
            ...
            'help'  => 'user_class_completion_summarydatafields_help',
            'fields' => array(
                'userfields' => array(
        ...
        $detailheadoptions = array(
            ...
            'name'  => 'detail_headers',
            'lang'  => $this->languagefile,
            'help'  => 'user_class_completion_detailheaderfields_help',
            'fields' => array(
                'userfields' => array(
                    'user_address' => 'address',
                    'user_city'    => 'city',
        ...
        $detailcoloptions = array(
            ...
            'name'  => 'detail_fields',
            'lang'  => $this->languagefile,
            'help'  => 'user_class_completion_detaildatafields_help',
            'fields' => array(
                'curriculumfields' => array(
                    'cur_name'          => 'curriculum',
                    'custom'            => array('curriculum' => 'all'),
        ...
        $schemas = array(
                ...
                array('completionstatus', 'filter_completionstatus', false,
                      'nofiltersimpleselect', $completionstatus_options),
        ...
        foreach ($schemas as $schema) {
            if (is_array($schema)) {
                ...
                $filters[] = new generalized_filter_entry('filter-'. $schema[0], '', $schema[0], $label,
        ...
        if (!empty($cols)) {
            foreach ($cols as $field => $active) {
                if ($active && (substr($field, 0, 7) == 'custom_')) {
                    ...
                    $name = get_field('crlm_field', 'name', 'id', $fieldid);
                    ...
                    if ($default_records = $CURMAN->db->get_records_select($field->data_table(), "contextid IS NULL AND fieldid = {$field->id}")) {
                    ...
                    if (record_exists('crlm_field_contextlevel', 'fieldid', $fieldid, 'contextlevel', $level)) {
    ...
    function get_class_join_sql($on_or_where = 'ON') {
        ...
        if (!empty($ccc_sql)) {
            $join = 'JOIN '. $CURMAN->db->prefix_table('crlm_class_enrolment') .' cce';
            if ($on_or_where == 'ON') {
                $join .= ' ON u.id = cce.userid';
            }
            $join .= ' JOIN '. $CURMAN->db->prefix_table(CLSTABLE) .' cls'
    ...
    function get_custom_field_sql($fieldids, $instancefields) {
        ...
        if (!empty($contexts)) {
            ...
            foreach ($contexts as $context) {
                ...
                $fragment[] .= " LEFT JOIN {$CURMAN->db->prefix_table($field->data_table())} {$identifier}"
   ...
   function get_elements_sql() {
        ...
        $sql = "FROM {$CURMAN->db->prefix_table(CRSCOMPTABLE)} course_completion
                JOIN {$CURMAN->db->prefix_table(GRDTABLE)} class_graded
    ...
    function get_access_sql() {
        ...
        $access = "EXISTS (SELECT *
                           ...
                           JOIN {$CURMAN->db->prefix_table('log')} log
                             ON clsmdl.moodlecourseid = log.course
                           JOIN {$CURMAN->db->prefix_table('user')} mdlu
    ...
    function get_report_sql($columns) {
        ...
        if ($showcurricula) {
            ...
            $filter_clause = $this->get_filter_condition('', false);
    ...
    function transform_record($record, $export_format) {
        ...
        if ($export_format == php_report::$EXPORT_FORMAT_HTML) {
            ...
            if ($userpage->can_do()) {
                $record->lastname = '<span class="external_report_link"><a href="' . $userpage->get_url()
    ...
    function get_filter_condition($conditional_symbol, $include_clustertree = true) {
        ...
        if(!empty($this->filter)) {
            ...
            $sql_filter = $this->filter->get_sql_filter('', array(), $this->allow_interactive_filters(),
                                                        $this->allow_configured_filters(), '', $include_clustertree);
            if(!empty($sql_filter)) {
...
class clustertree_optional_filtering extends php_report_default_capable_filtering {
     ...
     * @return string
     */
    function get_sql_filter($extra='', $exceptions = array(), $allow_interactive_filters = false, $allow_configured_filters = true,
                            $secondary_filtering_key = '', $include_clustertree = true) {
        global $SESSION;
