...
class course_completion_by_cluster_report extends table_report {
    ...
    function get_filters($init_data = true) {
        ...
        $clustertree_help = array(
                'course_completion_by_cluster_help',
                $help_label,
                'block_php_report');
        ...
        $columns_filter = new generalized_filter_entry('columns', '', '', $columns_heading, false,
                                                       'nofiltercheckboxes',   $columns_options);
        ...
        $clusterrole_filter = new generalized_filter_entry('clusterrole', '', '', $clusterrole_heading,
                                                           false, 'nofiltersimpleselect', $clusterrole_options);

        //return all filters
        return array(
                $cluster_filter, $completion_filter, $columns_filter, $clusterrole_filter);
    }
    ...
    function transform_grouping_header_record($element, $datum, $export_format) {
        ...
        $induser_report_location = $CFG->wwwroot.'/blocks/php_report/render_report_page.php?'.
                                   'report=individual_user&userid='.$datum->userid;

        //make this a link if we're in HTML format
        if ($export_format == php_report::$EXPORT_FORMAT_HTML) {
            $element->useridnumber = '<span class="external_report_link"><a href="'.$induser_report_location.
                                      '"" target="_blank">'.
                                      $element->useridnumber.
                                      '</a></span>';
        }
        ...
        //make this a link if we're in HTML format
        if ($export_format == php_report::$EXPORT_FORMAT_HTML) {
            $element->firstname = '<span class="external_report_link"><a href="'.$induser_report_location.
                                  '"" target="_blank">'.
                                  $element->firstname.
                                  '</a></span>';
        }
