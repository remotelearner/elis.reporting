 ...
 *
 * @package    elis
 * @subpackage cm-blocks-phpreport-sitewide_time_summary
...
class sitewide_time_summary_report extends table_report {
    ...
    function require_dependencies() {
        ...
        require_once($CFG->dirroot . '/curriculum/lib/filtering/radiobuttons.php');
    ...
    function get_preferred_header_icon() {
        global $CFG;
        return $CFG->pixpath .'/php_reports/sitewide_time_summary_report_logo.jpg';
    }
    ...
    function get_report_sql($columns) {
        ...
        $sql = "SELECT $columns, u.firstname as firstname, u.lastname as lastname, clsm.moodlecourseid AS mcourse_id
            ...
            JOIN {$CFG->prefix}crlm_class cls ON cls.id = clsenr.classid";
        ...
        $sql .= "
            JOIN {$CFG->prefix}crlm_course crs ON cls.courseid = crs.id
            JOIN {$CFG->prefix}crlm_class_moodle clsm ON cls.id = clsm.classid";

        if ($this->segment == 'noseg') {
            $sql .= "
            WHERE EXISTS (
                SELECT id
                FROM mdl_etl_user_activity etlua
            ...
            $sql .= "
            JOIN {$CFG->prefix}etl_user_activity etlua
