...
class user_class_completion_details_report extends user_class_completion_report {
    ...
    function get_report_sql($columns) {
        ...
        if ($status_param = php_report_filtering_get_active_filter_values($parent_shortname, 'filter-completionstatus')) {
        ...
        if ($has_fields) {
            ...
            $curriculum_join    = " LEFT JOIN ({$CURMAN->db->prefix_table(CURCRSTABLE)} curcrs
                                  ...
                                  JOIN {$CURMAN->db->prefix_table(CURASSTABLE)} curass
                                    ...
                                    ON crs.id = curcrs.courseid AND curass.userid=u.id
                                    {$class_custom_field_join} ";
		}

        //obtain SQL conditions related to the parent report's curriculum-course-class filter
        $ccc_conditions = $this->get_ccc_conditions($has_fields);
        ...
        if ($this->is_filtering_by_curricula()) {
            ...
            if ($has_fields) {
                ...
                $student_curriculum_join = "JOIN {$CURMAN->db->prefix_table(CURASSTABLE)} curstu
        ...
        $total_elements_sql = "SELECT count(*)
                               FROM {$CURMAN->db->prefix_table(CRSCOMPTABLE)} course_completion
        ...
        $sql = "SELECT {$columns},
                ...
                SELECT {$columns},
                       ...
                       cls.id AS classid
                FROM {$CURMAN->db->prefix_table(USRTABLE)} u
                JOIN {$CURMAN->db->prefix_table(INSTABLE)} inst
                  ON u.id = inst.userid
                JOIN {$CURMAN->db->prefix_table(CLSTABLE)} cls
    ...
    function is_filtering_by_curricula() {
        $values = php_report_filtering_get_active_filter_values($this->parentname, 'filter-ccc-curriculum-name');
    ...
    function get_curr_customfield_filter($showing_curricula, $curriculumfield, $id) {
        ...
        $options['wrapper'] = " INNER JOIN {$CFG->prefix}crlm_curriculum_course ccc"
    ...
    function get_curr_customfield_condition($showing_curricula, $id, $curriculumfield) {
        ...
        if ($filter_object = $this->get_curr_customfield_filter($showing_curricula, $curriculumfield, $id)) {
            if ($data = $filter_object->check_data($data)) {
                if ($sql = $filter_object->get_sql_filter($data)) {
                    $condition = $sql;
                }
            }
        }
     ...
     * @return array The complete collection of conditions related to curriculum custom fields
     */
    function get_curr_customfield_conditions($showing_curricula) {
        ...
        $ctxtlvl = context_level_base::get_custom_context_level('curriculum', 'block_curr_admin');
    ...
    function get_ccc_conditions($showing_curricula) {
        ...
        $curr_customfield_conditions = $this->get_curr_customfield_conditions($showing_curricula);
    ...
    function get_ccc_select_clause($filtername, $dbfield) {
        ...
        $values = php_report_filtering_get_active_filter_values($this->parentname, $filtername);
    ...
    function get_curriculum_exists_clause() {
        ...
        if (!empty($values[0]['value'])) {
            ...
            return "EXISTS (SELECT * FROM
                            {$CURMAN->db->prefix_table(CURASSTABLE)} curstu
    ...
    function transform_record($record, $export_format) {
        ...
        if ($export_format == table_report::$EXPORT_FORMAT_HTML) {
            ...
            $page = new cmclasspage(array('id' => $record->classid,
                                          'action' => 'view'));
            if ($page->can_do()) {
                $url = $page->get_url();
    ...
    function get_total_credits($grouping_current, $grouping, $datum, $export_format) {
        ...
        if ($value = php_report_filtering_get_active_filter_values($this->parentname, 'filter-completionstatus')) {
        ...
        foreach ($this->filter->_fields as $key => $value) {
            if (strpos($key, 'filter-ccc-curriculum-customfield-') === 0) {
                ...
                if ($test_customfield = php_report_filtering_get_active_filter_values($this->get_report_shortname(), $key)) {
            ...
            if ($filtering_cur_customfield) {
                ...
                $noncurriculum_sql = "SELECT SUM(stu.credits)
                                      ...
                                      WHERE curstu.id IS NULL
                                        ...
                                        {$ccc_sql}
                                        AND {$permissions_filter}
                                        AND {$startdate_condition}"; // TBD: AND {$filter_clause}
                ...
                if ($filter_value) {
                    if (false !== ($pos = strpos($field_alias, '_'))) {
                        ...
                        if (0 == strcmp($custom_field_name, 'custom')) {
                            ...
                            if (record_exists('crlm_field_contextlevel', 'fieldid', $custom_field_id, 'contextlevel', $level)) {
                                $fields_included = true;
                                break;
                            }

                        } elseif ( 0 == strcmp($field_alias, 'cur_name') ) {
                                $fields_included = true;
                                break;
                        }
