...
class course_completion_gas_gauge_report extends gas_gauge_table_report {
    ...
    function get_num_pages_sql() {
        ...
        return "SELECT COUNT(*)
                FROM
                {$CURMAN->db->prefix_table(CRSTABLE)}
    ...
    function get_page_tooltip_sql() {
        ...
        $sql = "SELECT name
                FROM {$CURMAN->db->prefix_table(CRSTABLE)}
                WHERE {$permissions_filter}";
    ...
    function get_report_sql($columns) {
        ...
        return "SELECT {$columns}, u.lastname, COUNT(cc.id) AS numcompletionelements,
                ...
                JOIN {$CURMAN->db->prefix_table(STUTABLE)} stu
                  ON u.id = stu.userid
                JOIN {$CURMAN->db->prefix_table(CLSTABLE)} AS cls
                  ON cls.id = stu.classid
                JOIN {$CURMAN->db->prefix_table(CRSTABLE)} AS crs
                  ON cls.courseid = crs.id
                LEFT JOIN {$CURMAN->db->prefix_table(CRSCOMPTABLE)} cc
                  ON cc.courseid = crs.id
                LEFT JOIN {$CURMAN->db->prefix_table(CLSGRTABLE)} AS ccg
    ...
    function transform_record($record, $export_format) {
        ...
        if ($export_format == php_report::$EXPORT_FORMAT_HTML) {
            ...
            $record->firstname = '<span class="external_report_link"><a href="' . $userpage->get_url() .
                                 '" target="_blank">' . fullname($record) . '</a></span>';
        } else {
    ...
    function get_gas_gauge_value($key) {
        ...
        $base_sql = "SELECT COUNT(DISTINCT u.id)
                     ...
                     JOIN {$CURMAN->db->prefix_table(CRSTABLE)} AS crs
                       ON cls.courseid = crs.id
                     WHERE crs.id = {$this->gas_gauge_page_value}";
    ...
    function get_gas_gauge_header_info() {
        $course_name = '';
        if ($course_record = get_record(CRSTABLE, 'id', $this->gas_gauge_page_value)) {
