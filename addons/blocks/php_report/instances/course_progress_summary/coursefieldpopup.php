...
print_header($site->shortname . ': '.get_string('selectcustomfields','rlreport_course_progress_summary'));
...
if (empty($fieldsbycategory)) {
    ...
    $table = null;
    $columns = array(
        'category'    => get_string('category', 'rlreport_course_progress_summary'),
        'name'        => get_string('name', 'rlreport_course_progress_summary')
        );
    ...
    foreach ($fieldsbycategory as $category => $fields) {
        ...
        foreach ($fields as $field) {
            ...
            $newarr[] = '<a href="#" '.make_js_event($block_id, $field->id,$category.' - '.
                         $field->name,$fieldidlist,$fieldnamelist).' >'.$field->name.'</a>';
            $table->data[] = $newarr;
        }
    }
