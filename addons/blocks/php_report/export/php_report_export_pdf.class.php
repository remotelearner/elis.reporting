...
class php_report_export_pdf extends php_report_export {
    ...
    protected function update_pdf_column_sizes_from_data(&$newpdf, $query, &$widths, &$heights, &$found_record, &$min_widths) {
        ...
        if ($recordset = get_recordset_sql($query)) {
            while($datum = rs_fetch_next_record($recordset)) {
                $found_record = true;
    ...
    protected function render_pdf_column_headers(&$newpdf, $widths) {
        ...
        if (($newpdf->GetY() + $height) > ($newpdf->h - $newpdf->bMargin)) {
            //reset the vertical positioning
            $initial_y_offset = $newpdf->tMargin;
            $final_y_offset = $initial_y_offset + $height;
    ...
    function set_pdf_bold_status(&$newpdf, $bold) {
        ...
        $newpdf->SetFont($newpdf->FontFamily, $format_attribute, 0);
    }
    ...
    protected function render_pdf_core_data(&$newpdf, $query, $widths, $heights, &$need_columns_header) {
        ...
        if ($recordset = get_recordset_sql($query)) {
            ...
            $datum = rs_fetch_next_record($recordset);
            $next_datum = false;
            ...
            while ($datum !== false) {
                ...
                $next_datum = rs_fetch_next_record($recordset);
    ...
    protected function render_pdf_entry($newpdf, $datum, $widths, $height, $colour = NULL) {
        ...
        //manually break the page if the height exceeds the remaining space
        if (($newpdf->GetY() + $result) > ($newpdf->h - $newpdf->bMargin)) {
            //flush the page
            $newpdf->AddPage();
        }
