...
abstract class icon_report extends php_report {
    ...
    function display_icons($id) {
        ...
        //set data based on config values
        if($block_instance = get_record('block_instance', 'id', $id)) {
            $config_data = unserialize(base64_decode($block_instance->configdata));

            if(!empty($config_data->numcolumns)) {
                $this->numcolumns = $config_data->numcolumns;
            }

            if(!empty($config_data->iconwidth)) {
                $this->iconwidth = $config_data->iconwidth . 'px';
            }
        }
